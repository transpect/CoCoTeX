\begin{Heading}[label=chap:common]{chapter}
  \Title{coco-common.sty}
  \Subtitle{Commonly used methods that are not part of the {\CoCoTeX} Kernel}
\end{Heading}
...
\begin{Heading}[label=sec:set_props]{subsection}
  \Title{Setting and Using Property}\index{Properties!Setup}\index{Properties!Usage}
\end{Heading}
...
\lstinline{\ccSetPropertyX}\index[tex]{tpSetPropertyX@\texttt{\textbackslash
    tpSetPropertyX}} is another variant of \lstinline{\ccSetProperty}
...
When using \lstinline{\ccSetProperty}\index[tex]{SetProperty@\texttt{\textbackslash tpSetProperty}}, the second argument may not be
...
\begin{Heading}{subsection}
  \Title{Default Property Sets}\index{Properties!Default}\index{Default Property Set|textbf}
\end{Heading}
...
\begin{lstlisting}[style=tex]
\ccAddToDefault{<Namespace>}{<setProperties>}
\end{lstlisting}
where \lstinline{<setProperties>} is a list of \lstinline{\ccSetProperty}
assignments.\Hack{\newpage}
...
\begin{Heading}{subsection}
  \Title{Conditionals}\index{Properties!Conditionals}
\end{Heading}

There are two conditionals:
\begin{lstlisting}[style=tex]
\ccIfProp{<name>}{<then>}{<else>}
\ccIfPropVal{<name>}{<value>}{<then>}{<else>}
\end{lstlisting}
The first conditional \lstinline{\ccIfProp}\index[tex]{IfProp@\texttt{\textbackslash tpIfProp}} checks if a property with
...
The conditional \lstinline{\ccIfPropVal}\index[tex]{IfPropVal@\texttt{\textbackslash tpIfPropVal}} also checks if the Property
...
\begin{Heading}{subsection}
  \Title{Evaluation and Inheritance of Properties}\index{Properties!Evaluation}\index{Properties!Inheritance}
\end{Heading}

The evaluation of macros is done with the macro\index[tex]{CascadeProp@\texttt{\textbackslash tpCascadeProp}}:
\begin{lstlisting}[style=tex]
\ccCascadeProps{<namespace>}{<top-level-namespace>}
\end{lstlisting}
...
\begin{Heading}[label=sec:common:components]{section}
  \Title{Using Components}\index{Component!Usage}
\end{Heading}
...
\begin{Heading}[label=sec:common:components]{section}
  \Title{Using Hooks}\index{Hooks!Usage}
\end{Heading}
...
A Hook is declared with the \lstinline{\ccDeclareHook} macro which
takes the Hook's name as argument:
\begin{lstlisting}
\ccDeclareHook{<Hook-Name>}
\end{lstlisting}

To add code to a hook, simply use
\begin{lstlisting}[style=tex]
\ccAddToHook[<namespace>]{<Hook-Name>}{<Appendix}
\end{lstlisting}
...
\ccUseHook{<Hook-name>}
\end{lstlisting}
Make sure that \lstinline{\cc@namespace} is set correctly or else you
