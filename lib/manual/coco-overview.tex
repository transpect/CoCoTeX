\begin{Heading}{chapter}
  \Title{Overview}
  \Subtitle{Getting started with \CoCoTeX}
...
\begin{description}
\item[\ttfamily coco-kernel*] \textit{requires} \texttt{etoolbox}
\item[\ttfamily coco-common*] \textit{requires} \texttt{coco-kernel},
  \texttt{xcolor}, and \texttt{graphicx}
\item[\ttfamily coco-floats*] \textit{requires} \texttt{coco-common}
  and \texttt{rotating}, \texttt{grffile}, \texttt{footnote},
  \texttt{adjustbox}, and \texttt{stfloats} and \textit{supports}
...
\item[\ttfamily coco-meta*] \textit{requires} \texttt{coco-common}
\item[\ttfamily coco-heading*] \textit{requires} \texttt{coco-meta}, and
  \texttt{bookmark}
\item[\ttfamily coco-notes*] \textit{requires} \texttt{footnote}, and \texttt{endnotes}
\item[\ttfamily coco-title*] \textit{requires} \texttt{coco-meta}
\item[\ttfamily coco-accessibility] \textit{requires} lua\LaTeX, \texttt{coco-kernel},
  and \texttt{ltpdfa}\footnote{\texttt{ltpfa} is included in the
    ...
    package}. Older {\LaTeX} kernel versions \textit{require}
  \texttt{atbegshi}, \texttt{xparse}, \texttt{luatexbase-attr}, and
  \texttt{atveryend}
\item[\ttfamily coco-lists] \textit{requires} \texttt{coco-common}, \texttt{footnote},
  and \texttt{endnotes}
\item[\ttfamily coco-frame] \textit{requires} \texttt{luatex85}, and \texttt{crop}
\item[\ttfamily coco-script] \textit{requires} \texttt{coco-kernel}, \texttt{babel},
...
those modules are either not loaded (\texttt{coco-script}), or it is
loaded but not activated (\texttt{coco-accessibility}) by default.
