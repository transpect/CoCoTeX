...
called an \MainTerm{Instance} of that Container. For example, the
(particular) heading with the (particular) title “Introduction” is an
Instance of the abstract concept (or “Container“) “Heading”.

In {\CoCoTeX}, Containers are (usually) LaTeX environments that are
...
\textit{Container}) \ContainerString{Heading}.
...
\end{Figure}
\end{lstlisting}
is an Instance of the Container \ContainerString{Figure}, but Child
Containers of the \ContainerString{Heading} Parent are instantiated by
...
\end{Heading}
\end{lstlisting}
This is an instance of the Container \ContainerString{chapter}, which
in turn is a Child Container of the Parent Container
\ContainerString{Heading}. Different modules of the {\CoCoTeX}
...
\begin{Heading}{subsubsection}
  ...
  \RefLabel{sec:components:usage:simple}
\end{Heading}
...
consists of two Components, \ComponentString{Caption} and
...
name in \InlineArg{1} already exists. If no such Component exists, it
...
\begin{Heading}{subsubsection}
  ...
  \RefLabel{sec:components:usage:cg}
\end{Heading}
...
\end{Heading}
\end{lstlisting}
In this example, the Container \ContainerString{Section} is
instantiated\footnote{for the special syntax of
  \ContainerString{Heading}'s Child Container Instances,
  see~\fullref{sec:inheritance} and \fullref{chap:headings}} with
three Components: one instance of the simple Component
\ComponentString{Title} that is the value \textit{Section Title}
assigned to, and two instances of the Counted Component
\ComponentString{Author}. The first Instance has the values
...
\ComponentString{FirstName} and
\ComponentString{LastName}. \ComponentString{FirstName} and
\ComponentString{LastName} are both counted components, because they
can (and do) occur more than once in the enclosing Container.

When processed, the Values of the Counted Components
\ComponentString{FirstName} and \ComponentString{LastName} are
combined in a special way (more on that later), and the result is
stored in a generated Collection Component \ComponentString{FullName}
inside each instance of the Group Component
\ComponentString{Author}. Since Collection Components are also
...
how to build the Collection Component \ComponentString{FullName}
...
\PropertyString{title-format}. By convention, Property names are
always lower case and words are separated by hyphen.


\begin{Heading}{subsection}
  \Title{How to Use Properties}
  \RefLabel{sec:properties:usage}
\end{Heading}
...
\begin{Heading}{subsubsection}
  \Title{Defining and Using Properties}
  \RefLabel{sec:properties:useprops}
\end{Heading}
...
Sometimes, a pre-defined Property's purpose is it to call another
...
two Properties as its arguments, and makes Property \InlineArg{1} an
alias of Property \InlineArg{2}:
\begin{lstlisting}
\ccPropertyLet{toc-number-face}{toc-title-face}
\end{lstlisting}
in this case, the font of a heading's \ComponentString{Number} in the
table of contents is set to be the same as the font of the entries
\ComponentString{Title}. If we change the
\PropertyString{toc-title-face} Property afterwards,
\PropertyString{toc-number-face} changes as well, as the line above is
equivalent to
\begin{lstlisting}
...
\lstinline{\edef}) the Property in \InlineArg{2} before its remainder
is assigned to the Property in \InlineArg{1}. If we change
\PropertyString{toc-title-face} after using
\begin{lstlisting}
\ccPropertyLetX{toc-number-face}{toc-title-face}
\end{lstlisting}
the Property \PropertyString{toc-number-face} retains the original
value of \PropertyString{toc-title-face}.
...
\begin{Heading}{subsubsection}
  \Title{Calling Component Values}
  \RefLabel{sec:properties:usecomp}
\end{Heading}
...
\InlineArg{1} the name of a Component and is expanded to the value
that is assigned to that Component.

A macro similar to \UsageMacro{\ccWhenComp} is
\DeclareMacro{\ccIfComp}, which takes three arguments: \InlineArg{1}
...
\begin{Heading}{section}
  \Title{Optional Macro and Environment Prefix}
  \RefLabel{sec:design:prefix}
\end{Heading}

If you use {\CoCoTeX} in conjunction with \textit{xerif}, you might
...
option\SeeIndex{prefix}{Class Options}\index{Class Options>prefix}
...
\begin{myFigure}
  ...
  \ccComponent{Fig}{\includegraphics{example.eps}} % uses the existing "Fig" Component
\end{myFigure}
\end{lstlisting}
...
\begin{myFigure}
  ...
  \ccComponent{myFig}{\includegraphics{example.eps}} % generates a new Component "myFig"
\end{myFigure}
\end{lstlisting}
