...
function tree.analyze_node(head, level)
  ...
  for _, field_name in pairs(node.fields(head.id, head.subtype)) do
     if field_name == 'attr' then
        ...
        --[[
           had to exclude whatsit write nodes 'data' field because head[field_name] 
           crashes if stream number 1
        ]]--
     elseif head.id == 8 and head.subtype == 1 and field_name == 'data' and head['stream'] == 1 then
        --local tmp = head['data']
        --log("ignored write") -- head['data'] node.is_node(head[field_name])
        output_fields = output_fields .. 'ignored write '
